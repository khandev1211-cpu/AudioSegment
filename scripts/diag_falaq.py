"""Diagnose word-stream + phrase matching for Surah 113 (Al-Falaq)."""
import sys
import json
...
for qari in sys.argv[1:] or ["Reciter Bandar Balila"]:
    y, sr16 = load_audio(f"Surah/113/{qari}/113.mp3", sr=SR)
    ...
    for i, w in enumerate(words):
        print(f"     {i:2d}  {w['start_sec']:6.2f}-{w['end_sec']:6.2f}  '{w['text']}'  norm='{w['norm']}'  p={w['probability']}")

    ref_segments = get_surah_meta(113)["reference_segments"]
